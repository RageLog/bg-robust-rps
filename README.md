...
│   ├── global_cross_dataset_evaluation.json     Zero-shot HaGRID per-model scores (V1)
│   ├── global_cross_dataset_evaluation_v2.json  V2 abstention probe (T16 source)
│   └── multiseed_summary.json   Multi-seed stability, seeds 42/123/2024 (T17 source)
├── paper/
...
# --- Resubmission additions (revision) ---
python src/build_external_test_v2.py                 # V2 abstention probe -> reports/aggregated/global_cross_dataset_evaluation_v2.json
python paper/scripts/make_none_robustness_table.py   # T16 (abstention robustness; reads V1 + V2 JSONs)
python src/verify_multiseed_splits.py                # leakage pre-flight for the multi-seed run
python src/train_multiseed.py                        # re-trains the FULL subset under seeds 42,123,2024 (additive; main code unchanged)
python src/aggregate_multiseed.py                    # -> reports/aggregated/multiseed_summary.json
python paper/scripts/make_multiseed_table.py         # T17 (seed stability)
```

> **Note.** `paper/main.tex` is the preprint working copy (IEEEtran). The
> manuscript submitted to *The Visual Computer* uses the Springer `sn-jnl`
> class and additionally includes the revision tables **T16** (abstention
> robustness) and **T17** (seed stability) produced by the resubmission
