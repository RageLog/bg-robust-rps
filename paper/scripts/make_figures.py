...
def f05_topcfg_confusion():
    ...
    base = ROOT.parent / "007_public_repo_v2" / "study" / "reports"
...
def f07_gradcam_grid():
    ...
    """
    rep = ROOT.parent / "007_public_repo_v2" / "study" / "reports"
    backbones = ["DenseNet121", "EfficientNetV2B0", "ResNet50",
                 "MobileNetV3Small", "VGG16"]
    correct = {
...
def f09_zeroshot_classwise_bars():
    ...
    """
    j = json.loads((ROOT.parent / "005_dev_code" / "reports"
                    / "cross_dataset"
                    / "global_cross_dataset_evaluation.json").read_text())
    mr = j["model_results"]
    backbones = ["DenseNet121", "EfficientNetV2B0", "ResNet50",
                 "MobileNetV3Small", "VGG16"]
    classes = ["rock", "paper", "scissors", "none"]
