...
RAW_CSV = ROOT / "reports" / "aggregated" / "all_per_fold_results.csv"
OUT_DIR = ROOT / "reports" / "aggregated"
...
def main():
    ...
    md.append(f"Source: `{RAW_CSV.relative_to(ROOT).as_posix()}`  ({len(df)} per-fold rows, {df['config'].nunique()} configs)\n")
